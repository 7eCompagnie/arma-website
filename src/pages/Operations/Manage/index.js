...
function OperationsManage() {
                ...
                <td>
                    <Center>
                        <Button color="yellow" size="md" compact
                                onClick={() => navigate(`/zeus/operations/${operation._id}`)}>
                            Editer
                        </Button>
    ...
    return (<>
        <h1>Gérer les opérations ({rows.filter(r => r !== null).length})</h1>
        ...
        {!rows.filter(r => r !== null).length ? <Alert icon={<AlertCircle size={16} />} title="Pas de données" mt={10} color={"red"}>
